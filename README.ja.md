...
macOS メニューバーに Claude の使用量をリアルタイム表示し、使い切る前にバーンレートで警告する軽量アプリです。
...
- **バーンレート警告** — 今のペースだと上限に届きそうなとき、使い切る前に警告
...
メニューバーには全アカウントのセッション利用率（例: `5% · 59%`）が、消費ペース予測で色づけされて常駐表示されます（いずれかのアカウントがリセット前に使い切るペースだとアイコンが警告表示に変わります）。アイコンをクリックすると、予測（「このペースで HH:mm に上限到達」をセッション行の上に表示）を含む詳細を確認できます。
...
ClaudeMeter は、現在の使用率だけでアイコン色を決めているわけではありません。5時間セッションについては、使用量エンドポイントが返す現在の利用率とリセット時刻から、いまの消費ペース（バーンレート）を推定します。

```text
経過時間       = 5時間 - リセットまでの残り時間
バーンレート   = 現在の利用率 / 経過時間
リセット時予測 = バーンレート × 5時間
```

この予測が警告/危険しきい値を超える場合、または現在のペースだとリセット前に 100% に到達する場合、メニューバーの色とアイコンを変えて知らせます。ポップオーバーには「このペースで 14:08 に上限到達」や「このペースだとリセット時 約82%」のような予測を表示します。

Claude の 5時間枠はローリングウィンドウなので、この予測は正確な未来予測ではなく、現在の平均ペースにもとづく早めの警告です。小さな初期サンプルで過剰に反応しないよう、ウィンドウ開始から30分経つまでは予測ではなく現在値ベースで判定します。
